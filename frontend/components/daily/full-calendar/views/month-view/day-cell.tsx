...
import { isToday, startOfDay, isSunday, subDays } from "date-fns";
...
import type { ICalendarCell, IEvent } from "@/components/daily/full-calendar/interfaces";
import { Button } from "@/components/ui/button";
import { Plus, CheckCircle2, Circle, MoreHorizontal, FileText, ClipboardList, History as HistoryIcon } from "lucide-react";
import {
  ContextMenu,
  ContextMenuContent,
  ContextMenuItem,
...
import { useCreateDailyLog, useCreateDailyTask, useUpdateDailyTask, useDailyLogs } from "@/hooks/use-daily";
...
export function DayCell({ cell, events }: IProps) {
  ...
  const handleQuickAdd = async (e: React.FormEvent) => {
    ...
    await createTask.mutateAsync({ logId: targetLogId, text: quickTaskText.trim() });
    setQuickTaskText("");
  ...
  const handleMoveUnfinished = async () => {
    ...
    if (tomorrowLog) {
      ...
      const res = await createLog.mutateAsync({ date: tomorrowStr, workspaceId });
      targetLogId = res.log.id;
    }
  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger asChild>
        ...
        >
          <DroppableArea date={date} className="w-full h-full p-2 flex flex-col gap-2">
            ...
            <div className="flex-1 space-y-1 overflow-hidden">
              {tasks.slice(0, 3).map((task: any) => (
                 <div key={task.id} className="flex items-center gap-1.5 min-w-0">
                    ...
                    <span className={cn(
                      ...
                      task.is_completed ? "text-muted-foreground/40 line-through" : "text-muted-foreground group-hover/cell:text-foreground/80"
                    )}>
                      {task.text}
                    </span>
                 </div>
              ...
              {tasks.length === 0 && !isQuickAddOpen && (
                 <div className="h-full flex items-center justify-center opacity-0 group-hover/cell:opacity-100 transition-opacity">
                    <Button variant="ghost" size="icon" className="size-6 rounded-full" onClick={(e) => { e.stopPropagation(); setIsQuickAddOpen(true); }}>
      ...
      <ContextMenuContent className="w-56 rounded-xl">
        ...
        {!logData && (
          <ContextMenuItem className="gap-2" onClick={() => createLog.mutate({ date: dateStr, workspaceId })}>
             <Plus className="size-4" />
             Create Record
          </ContextMenuItem>
        )}
        <ContextMenuSeparator />
        <ContextMenuItem className="gap-2" onClick={() => setIsQuickAddOpen(true)}>
