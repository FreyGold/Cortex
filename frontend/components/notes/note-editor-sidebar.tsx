...
function PropertyRow({ icon, label, children, className }: PropertyRowProps) {
  return (
    <div
      className={cn(
        "group flex items-center gap-3 py-1.5 px-2 rounded-md hover:bg-muted/40 transition-colors",
        className,
      )}
    >
      ...
      <div className="flex-1 min-w-0 text-xs truncate">{children}</div>
    </div>
  );
}
...
export function SharingDialog({
  ...
  updateNote,
  isPublished,
}: SharingDialogProps & { updateNote?: any; isPublished?: boolean }) {
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button
          variant="ghost"
          className="h-8 w-full justify-start gap-2 text-xs font-normal text-muted-foreground hover:text-foreground hover:bg-muted/40 px-2"
        >
      ...
      <DialogContent className="sm:max-w-md p-0 overflow-hidden rounded-2xl border-none shadow-2xl">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between py-1">
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-muted-foreground">
                Allows full collaborative editing.
              </p>
            </div>
            <input
              type="checkbox"
          ...
          >
            {isPending || (updateNote && updateNote.isPending)
              ? "Sharing..."
              : shareMode === "link"
                ? "Generate & Copy Link"
                : "Invite User"}
          ...
          {isPublished && shareMode === "link" && updateNote && (
            <div className="text-[11px] text-center text-primary font-bold py-2 bg-primary/5 rounded-lg animate-in fade-in zoom-in-95 flex items-center justify-center gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  updateNote.mutate({ isPublished: false });
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="space-y-2 max-h-[160px] overflow-y-auto custom-scrollbar pr-1">
                {shares.map((s: any) => (
                  <div
                    key={s.id}
                    className="flex items-center justify-between text-[11px] bg-muted/20 p-2.5 rounded-xl border border-border/5"
                  >
                    <span className="font-medium opacity-80 flex items-center gap-2 overflow-hidden">
                      <Users className="size-3 shrink-0" />
                      <span className="truncate">
                        {s.shared_with_user_id
                          ? `User: ${s.shared_with_user_id}`
                          : "Unknown"}
                      </span>
                      {s.can_edit && (
                        <Badge
                          variant="outline"
                          className="h-4 text-[8px] bg-emerald-500/10 text-emerald-500 border-none px-1 uppercase tracking-tighter"
                        >
                          Editor
                        </Badge>
                      )}
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-6 text-[10px] text-destructive hover:bg-destructive/10 rounded-lg px-2"
          ...
          <Dialog>
            ...
            <DialogContent className="p-0 overflow-hidden rounded-2xl border-none shadow-2xl sm:max-w-md">
              <DialogHeader className="p-6 pb-0">
                ...
                <DialogDescription>
                  Add this note as a resource to a university course.
                </DialogDescription>
              </DialogHeader>
              <div className="p-6">
          ...
          <div className="space-y-0.5">
            ...
            <PropertyRow icon={<FolderIcon />} label="Folder">
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="ghost"
                    className="h-6 w-full justify-start text-[11px] font-medium px-1 -mx-1 hover:bg-muted/60 transition-colors"
                  >
                    {note.folder_id
                ...
                <PopoverContent
                  className="w-[200px] p-0 shadow-xl border-border/40"
                  align="start"
                >
                  <Command>
                    <CommandInput
                    ...
                    <CommandList>
                      <CommandGroup>
                        <CommandItem
                          className="text-[11px]"
                          onSelect={() => onUpdateNote({ folderId: null })}
                        >
                          Root
                        </CommandItem>
                        {folders.map((f: any) => (
                          <CommandItem
                            key={f.id}
                            className="text-[11px]"
                            onSelect={() => onUpdateNote({ folderId: f.id })}
                          >
                            {f.name}
                          </CommandItem>
                        ))}
            ...
            <PropertyRow icon={<TagIcon />} label="Tags">
              <Popover>
                <PopoverTrigger asChild>
                  <div className="flex flex-wrap gap-1 cursor-pointer min-h-6 py-1 group/tags">
                    {selectedTagIds.length > 0 ? (
                        ...
                        return (
                          t && (
                            <Badge
                              key={id}
                              variant="secondary"
                              className="px-1.5 py-0 h-4 text-[9px] font-bold rounded bg-muted text-muted-foreground border-none uppercase tracking-tight"
                            >
                              {t.name}
                            </Badge>
                          )
                      ...
                      <span className="text-[10px] text-muted-foreground/30 italic group-hover/tags:text-muted-foreground transition-colors">
                        Add tags...
                      </span>
                    )}
                  </div>
                ...
                <PopoverContent
                  className="w-[240px] p-0 shadow-xl border-border/40"
                  align="start"
                >
                  <Command>
                    <CommandInput
                        ...
                        {tags.map((tag: any) => {
                          ...
                          return (
                            ...
                            >
                              <div
                                className={cn(
                                  "mr-2 size-2 rounded-full border",
                                  isSelected
          ...
          <div className="space-y-3 pt-4 border-t border-border/10">
            ...
            <div className="flex flex-col gap-1 px-1">
              <Button
                size="sm"
                variant="ghost"
                className="h-8 justify-start text-[11px] font-medium text-muted-foreground hover:text-foreground px-2"
              >
                Analyze document
              </Button>
              <Button
        ...
        <div className="flex items-center gap-3">
          <div className="flex -space-x-1.5 overflow-hidden">
            {[1, 2].map((i) => (
              <div
                key={i}
                className="inline-block size-5 rounded-full border border-background bg-muted-foreground/20 ring-1 ring-border"
              />
            ))}
          </div>
          <span className="text-[9px] font-bold text-muted-foreground/40 uppercase tracking-tighter">
            {shares.length > 0
              ? `${shares.length} Active Collaborators`
              : "Private Workspace"}
          </span>
        </div>
      </div>
