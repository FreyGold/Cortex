...
export function GlobalAssistantModal({ 
  ...
  const { mutateAsync: askNoteScoped, isPending: isAskingNote } = useAskNote(noteId || "");
  const isAsking = isAskingAll || isAskingNote;
  
  const { mutateAsync: fetchConversation } = useGlobalConversation();
  const { data: history = [], refetch: refreshHistory } = useListGlobalConversations(noteId);
  ...
  const latestNoteSessionId = useMemo(() => {
    ...
    return history.find((s: any) => s.note_id === noteId && s.status !== 'archived')?.id || null;
  }, [history, noteId]);
  ...
  const handleArchiveSession = useCallback(async (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    try {
      await archiveConversation(id);
      if (activeId === id) handleNewChat();
      refreshHistory();
      toast.success("Conversation archived", { description: "Auto-deleted in 3 days." });
    } catch (e) {
          ...
          {isHistoryOpen && (
            ...
            >
              <div className="p-4 border-b border-border/10 shrink-0">
                <Button variant="outline" className="w-full justify-start h-9 bg-primary/5 hover:bg-primary/10 border-primary/20" onClick={handleNewChat} disabled={isAsking}>
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-1">
                <div className="text-[10px] font-bold text-muted-foreground mb-3 px-2 uppercase tracking-widest opacity-50">
                  {showArchived ? "Archived Chats (3 Days Left)" : "Active Chats"}
                </div>
                {filteredHistory
                  .filter((s: any) => showArchived ? s.status === 'archived' : s.status !== 'archived')
                  .map((session: any) => (
                    <div 
                      key={session.id}
                      role="button" 
                      ...
                      <div className="flex flex-col min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1 ml-5">
                           <span className={`text-[8px] px-1 py-0.5 rounded border leading-none ${
                             ...
                             {session.note_id ? (session.note_title || "Note") : "All Notes"}
                           </span>
                ...
                {filteredHistory.filter((s: any) => showArchived ? s.status === 'archived' : s.status !== 'archived').length === 0 && (
                  <div className="py-8 text-center px-4">
                    <p className="text-[11px] text-muted-foreground opacity-40 italic">
                      {showArchived ? "No archived chats" : "Your chat history will appear here"}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="p-3 border-t border-border/10 bg-muted/5">
                ...
                >
                  <div className="flex items-center gap-2">
                    {showArchived ? <Plus className="size-3.5 opacity-40 rotate-45" /> : <Trash2 className="size-3.5 opacity-40" />}
                    <span className="text-xs font-medium">{showArchived ? "Back to Active" : "Archive"}</span>
                  </div>
                  <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded-full min-w-5 group-hover:bg-muted/20">
                    {filteredHistory.filter((s: any) => s.status === 'archived').length}
                  </span>
                </Button>
              </div>
            </motion.div>
        ...
        <div className="relative flex size-full overflow-hidden bg-background rounded-2xl border border-border/40 shadow-xl">
          ...
          <div className="flex-1 flex flex-col min-w-0 bg-background divide-y">
            <div className="px-4 py-4 flex items-center justify-between shadow-sm bg-muted/5 shrink-0 border-b border-border/5">
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground mr-1" onClick={() => setIsHistoryOpen(!isHistoryOpen)}>
                  <Sidebar className="size-4" />
                </Button>
                <h2 className="text-base font-semibold flex items-center gap-3 tracking-tight">
                  <span className="p-1.5 rounded-lg bg-primary/10 text-primary shadow-sm"><Sparkles className="size-4" /></span>
                        ...
                        <div>
                          ...
                          {message.reasoning && (
                            <ChainOfThought>
                              ...
                              <ChainOfThoughtContent>
                                <ChainOfThoughtStep label="Thought" status="complete">
              ...
              {isAsking && (
                <Message from="assistant" key="loading">
                  <MessageContent>
                    <ChainOfThought open>
                      <ChainOfThoughtHeader>
                         <Shimmer duration={1.5} className="font-medium">Analyzing library...</Shimmer>
                      </ChainOfThoughtHeader>
                      <ChainOfThoughtContent>
                        <ChainOfThoughtStep 
                          label="Searching note indices" 
                          status={loadingStage === 0 ? "active" : "complete"}
                          description={loadingStage === 0 ? "Scanning your connected knowledge base for relevant context..." : "Found potential matches across your notes"}
                        />
                        <ChainOfThoughtStep 
                          label="Evaluating context" 
                          status={loadingStage === 1 ? "active" : loadingStage > 1 ? "complete" : "pending"}
                          description={loadingStage === 1 ? "Ranking chunks and selecting the most pertinent information..." : loadingStage > 1 ? "Context optimized for response" : undefined}
                        />
                        <ChainOfThoughtStep 
                          label="Synthesizing response" 
                          status={loadingStage === 2 ? "active" : "pending"}
                          description={loadingStage === 2 ? "Compiling a cited answer based on your research..." : undefined}
                        />
                      </ChainOfThoughtContent>
                    </ChainOfThought>
                  </MessageContent>
