        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {notes.map((note) => (
            <Link
              ...
              className="group block h-full active:scale-[0.98] transition-all duration-150 ease-[cubic-bezier(0.16,1,0.3,1)]"
            >
                  ...
                  <div className="flex -space-x-1">
                    {(note as any).note_tags?.slice(0, 3).map((nt: any) => (
                      <div
                        ...
                        style={{ backgroundColor: nt.tags.color }}
                      />
                    ))}
                  </div>
                  ...
                  <div className="flex gap-1.5">
                    {(note as any).note_tags?.slice(0, 2).map((nt: any) => (
                      <Badge
                        key={nt.tags.id}
                        variant="outline"
                        className="text-[9px] uppercase font-bold h-5 px-1.5 border-border/40 text-muted-foreground/60"
                      >
                        {nt.tags.name}
                      </Badge>
                    ))}
