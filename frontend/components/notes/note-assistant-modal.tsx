          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-xl font-bold flex items-center gap-2 tracking-tight">
                ...
                <Badge
                  variant="secondary"
                  className="font-mono text-[10px] uppercase tracking-tighter h-5"
                >
                  Deep Research
                </Badge>
              </DialogTitle>
              ...
              <div className="p-4 space-y-3">
                {messages
                  ...
                  .map((m, idx) => (
                    <div
                      key={idx}
                      className="group relative p-4 rounded-2xl bg-background/40 border border-border/5 hover:border-primary/20 hover:bg-background transition-all cursor-pointer shadow-sm"
                    >
                      <p className="text-xs font-bold line-clamp-2 pr-4 leading-relaxed tracking-tight">
                        {m.content}
                      </p>
                ...
                {messages.length === 0 && !askMutation.isPending && (
                  <div className="py-32 text-center space-y-6 animate-in fade-in zoom-in-95 duration-1000">
                    ...
                    <div className="space-y-3">
                      <h4 className="text-3xl font-black tracking-tighter uppercase italic bg-clip-text text-transparent bg-gradient-to-br from-foreground to-foreground/50">
                        Note Companion
                      </h4>
                      <p className="text-base text-muted-foreground max-w-lg mx-auto leading-relaxed font-medium opacity-80 px-4">
                        Ask questions grounded in the context of this specific
                        research note.
                      </p>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    >
                      {m.role === "assistant" ? (
                              ...
                              <SourcesContent className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 w-full max-w-full mt-8">
                                {m.references.map((ref, idx) => (
                                  <Source
                                    key={idx}
                                    href="#"
                                    title={ref.heading || `Passage ${idx + 1}`}
                                  >
                        ...
                        <div className="text-2xl font-black py-2 tracking-tighter text-foreground leading-tight italic border-l-4 border-primary/20 pl-6 my-4 bg-primary/5 rounded-r-2xl py-6 pr-8">
                          {m.content}
                        </div>
                      )}
                    </MessageContent>
                  ...
                  <div className="flex gap-6 animate-in fade-in duration-500">
                    ...
                    <div className="space-y-4 w-full">
                      <Reasoning isStreaming={true}>
                        <ReasoningTrigger>
                          <span className="text-base font-black text-primary uppercase tracking-[0.2em] opacity-80 italic animate-pulse">
                            {reasoningStep}
                          </span>
                        </ReasoningTrigger>
                        <ReasoningContent className="text-sm font-medium text-muted-foreground/80 leading-relaxed">
                          The assistant is analyzing the note content and
                          retrieving the most relevant sections to provide a
                          factual answer based on your research...
                        </ReasoningContent>
                      </Reasoning>
                    </div>
                  </div>
