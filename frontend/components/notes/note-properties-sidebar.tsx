    ...
    >
      <Icon
        className={cn(
          ...
          destructive
            ? "text-destructive/50 group-hover:text-destructive/70"
        ...
        {detailsExpanded && (
          <div className="space-y-0.5 ml-1 mb-4">
            ...
            <Popover>
              ...
              <PopoverContent
                className="w-[200px] p-0 shadow-lg border-border/10 rounded-xl"
                align="end"
              >
                <Command>
                  <CommandInput
                  ...
                  />
                  <CommandList>
                    <CommandEmpty className="py-4 text-center text-xs text-muted-foreground italic">
                      No folders.
                    </CommandEmpty>
                    <CommandGroup>
                      <CommandItem
            ...
            <Popover>
                        ...
                        <div className="flex gap-1 justify-end overflow-hidden">
                          {selectedTagIds.slice(0, 2).map((id) => {
                            ...
                            return t ? (
                              <span
                                key={id}
                                className="px-1.5 bg-accent/60 text-foreground/80 text-[10px] font-semibold rounded uppercase tracking-wider"
                              >
                                {t.name}
                              </span>
                            ) : null;
              ...
              <PopoverContent
                className="w-[240px] p-0 shadow-lg border-border/10 rounded-xl"
                align="end"
              >
                <Command>
                  <CommandInput
        ...
        <div className="space-y-0.5 ml-1 mb-4">
          ...
          <Dialog>
            <DialogTrigger asChild>
              <div>
                <ActionItem icon={Share2} label="Share Note" />
              </div>
            </DialogTrigger>
            <DialogContent className="sm:max-w-md p-0 overflow-hidden rounded-2xl border-none shadow-2xl">
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="flex items-center justify-between py-1 px-1">
                  ...
                  <Select
                    value={shareRole}
                    onValueChange={(v: any) => setShareRole(v)}
                  >
                ...
                <Button
                  ...
                  disabled={
                    createShareMutation.isPending || updateNote.isPending
                  }
                >
                  {createShareMutation.isPending || updateNote.isPending
                    ? "Generating…"
                ...
                {isPublished && shareMode === "link" && (
                  <div className="text-[11px] text-center text-primary font-bold px-4 py-2 bg-primary/5 rounded-lg animate-in fade-in slide-in-from-top-1 flex items-center justify-center gap-2">
                    ...
                    <Button
                      variant="link"
                      className="h-auto p-0 text-primary text-[11px] font-bold"
                      onClick={async () =>
                    ...
                    <Button
                      variant="link"
                      className="h-auto p-0 text-destructive text-[11px]"
                      onClick={() => updateNote.mutate({ isPublished: false })}
                    >
                      Revoke
                    </Button>
                  </div>
                ...
                {shares?.length > 0 && shareMode === "user" && (
                  <div className="space-y-2 border-t border-border/20 pt-4">
                    <p className="text-[10px] font-extrabold uppercase tracking-widest text-muted-foreground/40 px-1">
                      Active
                    </p>
                    {shares.map((s: any) => (
                      <div
                        key={s.id}
                        className="flex items-center justify-between text-[11px] bg-muted/20 p-2 rounded-lg border border-border/5"
                      >
                        ...
                        <Button
                          variant="ghost"
                          className="h-6 text-[10px] text-destructive hover:bg-destructive/10 shrink-0"
          ...
          <Dialog open={resourceOpen} onOpenChange={setResourceOpen}>
            <DialogTrigger asChild>
              <div>
                <ActionItem icon={BookOpen} label="Publish as Resource" />
              </div>
            </DialogTrigger>
                  ...
                  <CommandList className="max-h-[300px] mt-2 p-1 custom-scrollbar">
                    ...
                    {!catalogLoading && courses?.length > 0 && (
                      <CommandGroup
                        ...
                        className="[&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:text-[10px] [&_[cmdk-group-heading]]:font-bold [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-widest [&_[cmdk-group-heading]]:text-muted-foreground/40"
                      >
                        {courses.map((c: any) => (
                          <CommandItem
                            key={c.id}
                          ...
                          >
                            <div className="flex flex-col items-start gap-1 min-w-0 pr-4">
                              ...
                              <Badge
                                variant="secondary"
                                className="h-5 px-1.5 text-[9px] font-mono tracking-wider bg-muted-foreground/10 text-muted-foreground group-aria-selected:bg-background border-none"
                              >
                                {c.code}
                              </Badge>
                            </div>
          ...
          {handleArchive && (
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <div>
                  <ActionItem icon={Trash2} label="Delete Note" destructive />
                </div>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Are you sure you want to archive this note? It will be moved
                    to the Trash where you can restore it or permanently delete
                    it later.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
            ...
            <AnimatePresence>
              {suggestedTagsText.length > 0 && (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="flex flex-wrap gap-1 mt-2 px-2"
                >
                  {suggestedTagsText.map((tag) => {
                    ...
                    const isSelected =
                      existingTag && selectedTagIds.includes(existingTag.id);
                    ...
                    return (
                      ...
                      >
                        {pendingTags.includes(tag) ? (
                          <RefreshCw className="size-3 animate-spin mr-1 inline-block" />
                        ) : (
                          "+ "
                        )}
                        {tag}
          ...
          <div className="flex items-center gap-2.5 px-2 mt-4">
            <div className="flex -space-x-1.5">
              {[...Array(Math.min(shares.length, 3))].map((_, i) => (
                <div
                  key={i}
                  className="size-[18px] rounded-full bg-muted-foreground/15 border-2 border-[#fbfbfa] dark:border-[#121212]"
                />
              ))}
            </div>
  ...
  if (isMobile) {
    return (
      <Sheet open={isOpen} onOpenChange={onOpenChange}>
        <SheetContent
          side="right"
          className="p-0 w-[300px] border-l border-border/5 bg-background"
        >
