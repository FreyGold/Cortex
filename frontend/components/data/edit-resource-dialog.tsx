...
import { Check, ChevronsUpDown, Pencil, Sparkles, Trash2 } from "lucide-react";
...
export function EditResourceDialog({
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      google_drive_url:
        resourceType === "note" ? null : google_drive_url || null,
    ...
    try {
      ...
      const res = await fetch(
        `${getBackendUrl()}/api/data/resources/${resource.id}`,
        {
          method: "PUT",
          headers: {
        ...
        <form onSubmit={onSubmit}>
            ...
            <div className="grid gap-5">
              ...
              {resourceType === "exam" && (
                <div className="grid gap-2">
                  ...
                  <Select
                    name="exam_type"
                    defaultValue={resource.exam_type || undefined}
                    required={resourceType === "exam"}
                  >
                    <SelectTrigger className="rounded-xl h-11 border-border/40">
              ...
              {resourceType !== "note" && (
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    defaultValue={resource.google_drive_url || ""}
                    placeholder="Link to file or folder"
                    className="rounded-xl h-11 border-border/40 focus:ring-primary/20"
                  />
                </div>
              )}
          ...
          <DialogFooter className="bg-muted/10 p-6 pt-4 border-t border-border/5 flex items-center justify-between sm:justify-between">
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="rounded-xl h-10 px-6 font-bold shadow-lg shadow-primary/10 transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
